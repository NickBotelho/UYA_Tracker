...
function GameHistory(props) {
    ...
    let startParamVal = query.get("start")
    ...
    let [games, setGames] = useState({
        start: startParamVal != null ? parseInt(startParamVal) : 0,
        games: null,
        max: 0,
    })
    const numEntries = 15
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    {games.start - numEntries < 0 ? null : <img src='../../server/build//backward_arrow.svg'
                        onMouseDown={() => {
                            props.history.push(`/gamehistory?start=${games.start - numEntries}`)
                            setGames({
                    ...
                    {games.start + numEntries > games.max ? null : <img src='../../server/build//forward_arrow.svg'
                        onMouseDown={() => {
                            // window.history.psuhState(null, "New Page Title",`/gamehistory?start=${games.start + numEntries}`)
                            props.history.push(`/gamehistory?start=${games.start + numEntries}`)
                            setGames({
                                start: games.start + numEntries,
                                games: null,
